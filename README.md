...
```
cd voxgrid
mkvirtualenv --no-site-packages voxgrid
...
```
* * * * * ubuntu bash /home/ubuntu/apps/voxgrid/repository/cron.sh fab $DEPLOYMENT_TARGET cron_jobs.test 
```
...
|--------|------|-----|-----|--------|--------|
|voxgrid|tweet|`location`||||
|voxgrid|facebook|`location`||||
|voxgrid|email|`location`||||
|voxgrid|new-comment||||
|voxgrid|open-share-discuss||||
|voxgrid|close-share-discuss||||
|voxgrid|summary-copied||||
|voxgrid|featured-tweet-action|`action`||``tweet_url``|
|voxgrid|featured-facebook-action|`action`||``post_url``|
